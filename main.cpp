...
void draw3DBox(const glm::vec3& entityPosition, const glm::mat4& projection, const glm::mat4& modelview, float boxWidth, float boxHeight, float boxDepth, const glm::vec4& color)
{
    ...
    glm::vec3 corners[8] = {
        { entityPosition.x - boxWidth / 2, entityPosition.y - boxHeight / 2, entityPosition.z - boxDepth / 2 },
        { entityPosition.x + boxWidth / 2, entityPosition.y - boxHeight / 2, entityPosition.z - boxDepth / 2 },
        { entityPosition.x + boxWidth / 2, entityPosition.y + boxHeight / 2, entityPosition.z - boxDepth / 2 },
        { entityPosition.x - boxWidth / 2, entityPosition.y + boxHeight / 2, entityPosition.z - boxDepth / 2 },
        { entityPosition.x - boxWidth / 2, entityPosition.y - boxHeight / 2, entityPosition.z + boxDepth / 2 },
...
void trigger()
{
    ...
    int nextCps = distrib(gentrigger);

}



void drawVisual(const glm::vec3& entityPosition, const glm::mat4& projection, const glm::mat4& modelview, Position& position, string tur)
{
    ...
    if (GUI::esp2dEnabled) {
        ...
        if (GUI::espModeSelected == 0) {
            ...
            for (int i = 0; i < sizeof(boxVertices) / sizeof(boxVertices[0]); i += 6) {
                glVertex3f(boxVertices[i], boxVertices[i + 1], boxVertices[i + 2]);
                glVertex3f(boxVertices[i + 3], boxVertices[i + 4], boxVertices[i + 5]);
            }
            glEnd();
        }
        else {
            ...
            float dynamicBoxHeight = baseSize / entityDistance;
 
            float dynamicBoxWidth = dynamicBoxHeight / 2.0f;
    ...
    if (GUI::chinaHatEnabled) {
        glm::vec3 chinahatTip = entityPosition + glm::vec3(0.0f, 1.0f, 0.0f);
        float hatRadius = 1.3f, hatHeight = 0.3f;
        ...
        SetColorChina(GUI::chinaR, GUI::chinaG, GUI::chinaB);

        glBegin(GL_TRIANGLE_FAN);
        glVertex3f(chinahatTip.x, chinahatTip.y - 2.1f, chinahatTip.z);
        for (int i = 0; i <= 360; i += 10) {
            float theta = glm::radians(float(i));
            float x = hatRadius * cosf(theta);
            float z = hatRadius * sinf(theta);
            glVertex3f(chinahatTip.x + x, chinahatTip.y - 2.1f + hatHeight, chinahatTip.z + z);
        }
        glEnd();
    ...
    if (zNear == 1000.0 && zFar == 3000.0)
    {
        ...
        for (auto& object : objects)
        {
            ...
            case Object::Chest:
                ...
                if (GUI::chestespEnabled) {
                    draw3DBox(entityPosition, projection, modelview, 0.5f, 0.5f, 0.5f, color);
                }
                break;
            ...
            case Object::LargeChest:
                ...
                if (GUI::chestespEnabled) {
                    draw3DBox(entityPosition, projection, modelview, 1.0f, 0.5f, 0.5f, color);
                }
                break;
            ...
            case Object::Item:
                ...
                if (GUI::itemespEnabled) {
                    draw3DBox(entityPosition, projection, modelview, 0.25f, 0.25f, 0.25f, color);
                }
